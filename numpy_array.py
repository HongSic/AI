...
#print(np.random.shuffle(arr)) # 기존 데이터 순서 바꾸기, data는 2개 이상이어야 함
...
return_index
값이 True면 입력 배열의 요소들의 인덱스들을 리턴합니다.
return_inverse
값이 True면 입력 배열을 재구성할 떄 쓰이는 고유 배열의 인덱스들을 반환합니다.
return_counts
값이 True면 중복 되지 않는 요소들이 입력 배열에 나타난 회 수를 리턴합니다.
'''
...
print ('배열요소의 인덱스:')
u, indices = np.unique(a, return_index=True) #값이 True면 입력 배열의 요소들의 인덱스들을 리턴합니다.
print (indices)
'''
'''
print ('고유 배열의 인덱스:')
u, indices = np.unique(a, return_inverse=True) #값이 True면 입력 배열을 재구성할 떄 쓰이는 고유 배열의 인덱스들을 반환합니다.
print (u)
...
u, indices = np.unique(a, return_counts=True) #값이 True면 중복 되지 않는 요소들이 입력 배열에 나타난 회 수를 리턴합니다.
print (u)
print (indices)
'''
...
#print(arr)
#print (arr.sum())  # 요소 전체 합
#print (arr.sum(axis=0))  # 요소 열 방향 합 ,  0는 행 방향 합
#print ((arr>0.5).sum())  #0.5보다 큰 값의 갯수
